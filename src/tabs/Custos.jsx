        ...
        <div style={{padding:"10px 14px",overflowX:"auto"}}>
          <table style={{width:"100%",minWidth:420}}>
            <thead><tr>
              ...
              <th style={{textAlign:"right",fontSize:11,color:C.muted,padding:"4px 6px"}}>Qtd/pessoa</th>
                ...
                <tr key={idx}>
                  ...
                  <td style={{padding:"3px 6px",textAlign:"right"}}>
                    <span style={{display:"inline-flex",alignItems:"center",gap:4}}>
                      <input type="number" step={step} min="0" value={l.g}
                        onChange={e=>updateLinha(prato.id,idx,"g",parseFloat(e.target.value)||0)}
                        style={{...inp,width:72,textAlign:"right"}}/>
                      <span style={{fontSize:11,color:C.muted,whiteSpace:"nowrap"}}>{lbl}</span>
                    </span>
                  </td>
                  <td style={{padding:"3px 6px",textAlign:"right",color:C.muted,fontVariantNumeric:"tabular-nums"}}>{un==="un"?"—":ins?Number(ins.fc).toFixed(2):"—"}</td>
...
export function Custos({insumos, insumoMap, pratos, custoLinha, custoPrato, updateInsumo, addInsumo, removeInsumo, ceasa, setCeasa, updatePrato, addPrato, removePrato, addLinha, updateLinha, removeLinha}) {
  ...
  return (<>
    ...
    {view==="insumos" && (<>
      ...
      <p style={{fontSize:13,color:C.muted,marginTop:-6}}><b>FC</b> = fator de correção (peso bruto ÷ líquido): cobre osso, casca e aparas — o que se compra a mais. Mexeu no preço, todos os pratos recalculam. · <b>Mín.</b> = estoque mínimo de segurança: quando o estoque cair abaixo desse valor, aparece um alerta na aba Operação. · Unidade <b>un</b> = unitário (ovo, pão etc.) — preço por unidade, FC não se aplica.</p>

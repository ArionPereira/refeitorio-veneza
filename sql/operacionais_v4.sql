...
declare v_tab text; v_lbl text; v_mod text; v_nome text;
begin
  v_tab := case p_entidade
    when 'item' then 'alm_itens' when 'categoria' then 'alm_categorias' when 'unidade' then 'alm_unidades'
  ...
  v_lbl := case p_entidade when 'veiculo' then 'identificacao' else 'nome' end;
  v_mod := case when p_entidade in ('veiculo','tanque') then 'abastecimento' else 'almoxarifado' end;
  execute format('update public.%I set ativo=$1 where id=$2 returning %I', v_tab, v_lbl)
